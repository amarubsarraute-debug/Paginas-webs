import { WHATSAPP_NUMBER, EMAIL, LOCATION, SCHEDULE, WHATSAPP_LINK, FACEBOOK_LINK, SOCIAL_LINKS } from "../lib/constants";
    ...
    <footer className="bg-bg-tint pt-20 pb-10 border-t border-border-subtle z-10 relative">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-5xl">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16 text-left">
          ...
          <div className="lg:col-span-2 space-y-6">
            <img
              ...
              className="h-14 w-[250px] sm:w-[310px] object-contain object-left"
            />
              ...
              {EMAIL && (
                <li>
                  <a href={`mailto:${EMAIL}`} className="flex items-center gap-3 text-muted hover:text-gold transition-colors font-semibold">
                    ...
                    <span className="truncate max-w-[180px] text-xs sm:text-sm">{EMAIL}</span>
                  </a>
                </li>
              )}
              {FACEBOOK_LINK && (
                <li>
                  <a href={FACEBOOK_LINK} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-muted hover:text-gold transition-colors font-semibold">
        ...
        <div className="pt-8 border-t border-border-subtle flex flex-col sm:flex-row items-center justify-between gap-4 font-mono text-xs text-muted">
          <p className="">
            &copy; {new Date().getFullYear()} Electropunta. Todos los derechos reservados.
          </p>
          <div className="font-semibold text-ink uppercase tracking-wider">
            Instalaciones Eléctricas Garantizadas
          </div>
        </div>
      </div>
    </footer>
