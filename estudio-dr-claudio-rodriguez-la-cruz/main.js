(function () {
  ...
  const safe = (fn, name) => {
    try {
      ...
      console.error(`[Rodríguez La Cruz] ${name}`, error);
    }
  };
  ...
  function initContactForm() {
    ...
    form.addEventListener('submit', (event) => {
      ...
      const message = [
        'Hola Dr. Claudio Rodríguez La Cruz, necesito realizar una consulta legal.',
        `Nombre: ${name}`,
        `Teléfono: ${phone}`,
        `Motivo: ${reason}`,
      ].join('\n');
      window.location.href = `https://wa.me/59899813012?text=${encodeURIComponent(message)}`;
    });
  }
